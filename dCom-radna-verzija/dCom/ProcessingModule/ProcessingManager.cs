...
namespace ProcessingModule
{
    ...
    public class ProcessingManager : IProcessingManager
    {
        ...
        /// <inheritdoc />
        public void ExecuteReadCommand(IConfigItem configItem, ushort transactionId, object remoteUnitAddress, ushort startAddress, ushort numberOfPoints)
        {
            // Attempt to cast remoteUnitAddress to byte, as expected by the other overload
            if (remoteUnitAddress is byte unitAddress)
            {
                ExecuteReadCommand(configItem, transactionId, unitAddress, startAddress, numberOfPoints);
            }
            else
            {
                throw new ArgumentException("remoteUnitAddress must be of type byte.", nameof(remoteUnitAddress));
            }
        }
        ...
        private void ProcessDigitalPoint(IDigitalPoint point, ushort newValue)
        {
            ...
            point.Alarm = alarmProcessor.GetAlarmForDigitalPoint(newValue, point.ConfigItem); //za alarm
        }
